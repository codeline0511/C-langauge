#define _CRT_SECURE_NO_WARNINGS // pre_order까지 작성 완료
#include <stdio.h>
#include <time.h>
#include <stdlib.h>
#include <string.h>
...
#define TABLE_SIZE 7
char* char_name[TABLE_SIZE] = { "soomoon", "charlie", "summer", "sean", "smu", "hacker", "jenny" };
...
int hash_chain_search(Member item, Member_List* hash_table[])
{
    ...
    for (node = hash_table[hash_value]; node; node = node->link)
    {
        if (!strcmp(node->item.name, item.name))
        {
            ...
            return node->item.order_count;
        }
    }
    printf("%s님은 등록된 회원이 아닙니다. 회원가입을 진행해 주세요\n", item.name);
    return -1;
}
...
int get_order(Owner* ice_owner)
{
    ...
    if (cus->bingsu_menu == 0) { pre_order(cus, bingsu_num); }
    if (cus->bingsu_menu == 1) { in_order(cus, bingsu_num); }
    if (cus->bingsu_menu == 2) { post_order(cus, bingsu_num); }
